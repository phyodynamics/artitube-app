...
export function AnimatedCopyButton({
  ...
  const handleCopy = async (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent trigger clicks on parent components
    try {
  ...
  return (
    <button
      onClick={handleCopy}
      className={cn(
        "relative flex items-center justify-center rounded-md border bg-background transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 z-20 cursor-pointer",
        sizes[size],
        className
      )}
      type="button"
